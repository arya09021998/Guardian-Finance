...
contract Guardian is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the GUARDIAN ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("GUARDIAN", "GUARDIAN") {
        // Mints 1 GUARDIAN to contract creator for initial pool setup
    ...
    function _getGuardianPrice() internal view returns (uint256 _guardianPrice) {
        try IOracle(guardianOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Guardian: failed to fetch GUARDIAN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _guardianPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_guardianPrice >= taxTiersTwaps[tierId]) {
    ...
    function setGuardianOracle(address _guardianOracle) public onlyOperatorOrTaxOffice {
        require(_guardianOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints GUARDIAN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of GUARDIAN to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentGuardianPrice = _getGuardianPrice();
            currentTaxRate = _updateTaxRate(currentGuardianPrice);
            if (currentGuardianPrice < burnThreshold) {
                burnTax = true;
            }
        }
