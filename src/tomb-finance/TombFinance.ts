...
/**
 * An API module of Guardian Finance contracts.
...
export class TombFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    this.GUARDIAN = new ERC20(deployments.tomb.address, provider, 'GUARDIAN');
    this.SHIELD = new ERC20(deployments.tShare.address, provider, 'SHIELD');
    this.GBOND = new ERC20(deployments.tBond.address, provider, 'GBOND');
    this.FTM = this.externalTokens['WFTM'];

    // Uniswap V2 Pair
    this.GUARDIANWFTM_LP = new Contract(externalTokens['GUARDIAN-FTM-LP'][0], IUniswapV2PairABI, provider);
  ...
  unlockWallet(provider: any, account: string) {
    ...
    const tokens = [this.GUARDIAN, this.SHIELD, this.GBOND, ...Object.values(this.externalTokens)];
    for (const token of tokens) {
      token.connect(this.signer);
    }
    this.GUARDIANWFTM_LP = this.GUARDIANWFTM_LP.connect(this.signer);
  ...
  async getTombStat(): Promise<TokenStat> {
    const { GuardianFtmRewardPool } = this.contracts;
    const supply = await this.GUARDIAN.totalSupply();
    const tombRewardPoolSupply = await this.GUARDIAN.balanceOf(GuardianFtmRewardPool.address);
    // const tombRewardPoolSupply2 = await this.GUARDIAN.balanceOf(GuardianFtmLPGuardianRewardPool.address);
    // const tombRewardPoolSupplyOld = await this.GUARDIAN.balanceOf(GuardianFtmLPGuardianRewardPoolOld.address);
    const tombCirculatingSupply = supply.sub(tombRewardPoolSupply);
      // .sub(tombRewardPoolSupply2);
      // .sub(tombRewardPoolSupplyOld);
    const priceInFTM = await this.getTokenPriceFromPancakeswap(this.GUARDIAN);
    ...
    return {
      ...
      totalSupply: getDisplayBalance(supply, this.GUARDIAN.decimal, 0),
      circulatingSupply: getDisplayBalance(tombCirculatingSupply, this.GUARDIAN.decimal, 0),
    };
  }
  ...
  async getLPStat(name: string): Promise<LPStat> {
    ...
    const token0 = name.startsWith('GUARDIAN') ? this.GUARDIAN : this.SHIELD;
    const isTomb = name.startsWith('GUARDIAN');
  ...
  async getShareStat(): Promise<TokenStat> {
    const { GuardianFtmLPShieldRewardPool } = this.contracts;

    const supply = await this.SHIELD.totalSupply();

    const priceInFTM = await this.getTokenPriceFromPancakeswap(this.SHIELD);
    const tombRewardPoolSupply = await this.SHIELD.balanceOf(GuardianFtmLPShieldRewardPool.address);
  ...
  async getTombStatInEstimatedTWAP(): Promise<TokenStat> {
    const { SeigniorageOracle, GuardianFtmRewardPool } = this.contracts;
    const expectedPrice = await SeigniorageOracle.twap(this.GUARDIAN.address, ethers.utils.parseEther('1'));
  ...
  async getGuardianPriceInLastTWAP(): Promise<BigNumber> {
    const { Treasury } = this.contracts;
    return Treasury.getGuardianUpdatedPrice();
  }

  async getBondsPurchasable(): Promise<BigNumber> {
    const { Treasury } = this.contracts;
    return Treasury.getBurnableGuardianLeft();
  }
  ...
  async getPoolAPRs(bank: Bank): Promise<PoolStats> {
    ...
    const stat = bank.earnTokenName === 'GUARDIAN' ? await this.getTombStat() : await this.getShareStat();
  ...
  async getTokenPerSecond(
    ...
    if (earnTokenName === 'GUARDIAN') {
      if (!contractName.endsWith('GuardianRewardPool')) {
        const rewardPerSecond = await poolContract.guardianPerSecond();
      ...
      if (Date.now() - startDateTime.getTime() > FOUR_DAYS) {
        return await poolContract.epochGuardianPerSecond(1);
      }
      return await poolContract.epochGuardianPerSecond(0);
    }
    const rewardPerSecond = await poolContract.ShieldPerSecond();
    if (depositTokenName.startsWith('GUARDIAN')) {
  ...
  async getDepositTokenPriceInDollars(tokenName: string, token: ERC20) {
    ...
    if (tokenName === 'WFTM') {
      ...
      if (tokenName === 'GUARDIAN-FTM-LP') {
        tokenPrice = await this.getLPTokenPrice(token, this.GUARDIAN, true);
      } else if (tokenName === 'SHIELD-FTM-LP') {
        tokenPrice = await this.getLPTokenPrice(token, this.SHIELD, false);
      } else if (tokenName === 'GUARDIAN-SHIELD-LP') {
        tokenPrice = await this.getLPTokenPrice(token, this.SHIELD, false);
      } else {
  ...
  async buyBonds(amount: string | number): Promise<TransactionResponse> {
    const { Treasury } = this.contracts;
    const treasuryTombPrice = await Treasury.getGuardianPrice();
  ...
  async redeemBonds(amount: string): Promise<TransactionResponse> {
    const { Treasury } = this.contracts;
    const priceForTomb = await Treasury.getGuardianPrice();
  ...
  async earnedFromBank(
    ...
    try {
      if (earnTokenName === 'GUARDIAN') {
        return await pool.pendingGUARDIAN(poolId, account);
      } else {
  ...
  async getGenesisPoolStartAndEndTime(): Promise<AllocationTime> {
    const { GuardianFtmRewardPool } = this.contracts;
    const startTimestamp: BigNumber = await GuardianFtmRewardPool.poolStartTime();
    const endTimestamp: BigNumber = await GuardianFtmRewardPool.poolEndTime();
  ...
  async watchAssetInMetamask(assetName: string): Promise<boolean> {
    ...
    if (ethereum && ethereum.networkVersion === config.chainId.toString()) {
      let asset;
      let assetUrl;
      if (assetName === 'GUARDIAN') {
        asset = this.GUARDIAN;
        assetUrl = 'https://raw.githubusercontent.com/krypto-dev/guardian-finance-assets/main/crypto_tomb_cash-svg.png';
      } else if (assetName === 'SHIELD') {
        asset = this.SHIELD;
        assetUrl = 'https://raw.githubusercontent.com/krypto-dev/guardian-finance-assets/main/crypto_tomb_share-svg.png';
      } else if (assetName === 'GBOND') {
        asset = this.GBOND;
        assetUrl = 'https://raw.githubusercontent.com/krypto-dev/guardian-finance-assets/main/crypto_tomb_bond-svg.png';
      }
  ...
  async quoteFromSpooky(tokenAmount: string, tokenName: string): Promise<string> {
    ...
    const { _reserve0, _reserve1 } = await this.GUARDIANWFTM_LP.getReserves();
    let quote;
    if (tokenName === 'GUARDIAN') {
  ...
  async estimateZapIn(tokenName: string, lpName: string, amount: string): Promise<number[]> {
    ...
    if (tokenName === FTM_TICKER) {
      ...
      const token = tokenName === TOMB_TICKER ? this.GUARDIAN : this.SHIELD;
  ...
  async swapTBondToTShare(tbondAmount: BigNumber): Promise<TransactionResponse> {
    ...
    return await TShareSwapper.swapGBondToShield(tbondAmount);
  }
  ...
  async getTShareSwapperStat(address: string): Promise<TShareSwapperStat> {
    ...
    const tbondBalanceBN = await TShareSwapper.getGBondBalance(address);
    // const tombPriceBN = await TShareSwapper.getGuardianPrice();
    // const tsharePriceBN = await TShareSwapper.getTSharePrice();
    const rateTSharePerTombBN = await TShareSwapper.getMeteprAmountPerGuardian();
